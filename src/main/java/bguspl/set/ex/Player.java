...
public class Player implements Runnable {
    ...
    public void point() {
        ...
        int seconds = (int) (env.config.pointFreezeMillis / ONESECOND);
        try {
            for (int i = seconds; i > 0; i--) {
                env.ui.setFreeze(id, i * ONESECOND);
                playerThread.sleep(ONESECOND);
            }
        } catch (InterruptedException e) {
        }
     ...
     */
    public void penalty() {
        int seconds = (int) (env.config.penaltyFreezeMillis / ONESECOND);
        for (int i = seconds; i > 0; i--) {
            env.ui.setFreeze(id, i * ONESECOND);
            try {
                playerThread.sleep(ONESECOND);
